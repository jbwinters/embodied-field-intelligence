...
<h1>EFI · Predictive crossing</h1>
<p id="description"></p>
<div class="panels">
<div><p>World · blue agent, red hazard, green goal</p><canvas id="world" width="330" height="330"></canvas></div>
<div><p>Hazard probability · next step</p><canvas id="near" width="330" height="330"></canvas></div>
...
<p>The dashed square is the agent’s observation window. Forecast panels use its discovered map;
...
document.getElementById('description').textContent=`Phase: ${data.result.phase}. Seed ${data.result.seed}, trial ${data.result.episode+1}. `+
`Success: ${data.result.success}. Return: ${data.result.return.toFixed(2)}. Rules learned online before and during this trial.`;
function draw(id,frame,field){
 ...
 for(let y=0;y<H;y++)for(let x=0;x<W;x++){
  ...
  if(field&&!walls[y][x]){ctx.fillStyle=`rgba(255,95,85,${Math.max(0,Math.min(1,field[y][x]))})`;ctx.fillRect(ox+x*s,oy+y*s,s-1,s-1);}
 }
 function dot(p,color,r){ctx.beginPath();ctx.arc(ox+(p[1]+.5)*s,oy+(p[0]+.5)*s,r*s,0,2*Math.PI);ctx.fillStyle=color;ctx.fill();}
 if(!field){dot(frame.goal,'#31cc89',.35);dot(frame.hazard,'#ff5f55',.35);}
...
function render(){const f=frames[index];scrub.value=index;draw('world',f,null);draw('near',f,f.forecast[0]);draw('far',f,f.forecast[f.forecast.length-1]);
 document.getElementById('farlabel').textContent=`Hazard probability · ${f.forecast.length} steps ahead`;
 document.getElementById('caption').textContent=`Step ${f.step} · chosen action: ${names[f.action]} · observed motion transitions learned: ${f.transitions}`;
